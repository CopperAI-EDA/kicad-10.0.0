...
SCH_EDIT_FRAME::SCH_EDIT_FRAME( KIWAY* aKiway, wxWindow* aParent ) :
        ...
        m_ollamaAgentPane( nullptr ),
        m_ollamaAgentNotebook( nullptr ),
        m_ollamaAgentTabPanel( nullptr ),
        m_datasheetTabPanel( nullptr ),
        m_ollamaAgentWebView( nullptr ),
        m_datasheetWebView( nullptr )
{
    ...
    // Always show the agent pane by default and initialize it via CallAfter so the
    // AUI perspective has been fully restored before we force-show and load the URL.
    CallAfter( [this]()
    {
                                            ...
                                            const VECTOR2I& aPos )
{
    ...
    if( SCH_LINE_WIRE_BUS_TOOL* lwbTool =
            GetToolManager()->GetTool<SCH_LINE_WIRE_BUS_TOOL>() )
    {
        lwbTool->BreakSegments( aCommit, aPos, aScreen );
    }
...
static wxString GetAgentChatWebviewUrl()
{
    ...
    return wxS( "https://getcopper.dev/chat?copper_client=kicad" );
#else
