...
static double ConnectionRotationDegreesFromSpin( SPIN_STYLE aSpin )
{
    switch( static_cast<int>( aSpin ) )
    {
    case SPIN_STYLE::RIGHT:  return 0.0;
    case SPIN_STYLE::UP:     return 90.0;
    case SPIN_STYLE::LEFT:   return 180.0;
    case SPIN_STYLE::BOTTOM: return 270.0;
    default:                 return 0.0;
    }
}

static void PackLabelBoundingBox( const BOX2I& aBBox,
                                   kiapi::schematic::types::LabelBBoxMm* aDest )
{
    if( !aDest )
        return;
    aDest->set_left_mm( (double) aBBox.GetLeft() / SCH_IU_PER_MM );
    aDest->set_right_mm( (double) aBBox.GetRight() / SCH_IU_PER_MM );
    aDest->set_top_mm( (double) aBBox.GetTop() / SCH_IU_PER_MM );
    aDest->set_bottom_mm( (double) aBBox.GetBottom() / SCH_IU_PER_MM );
}
...
void SCH_LABEL::Serialize( google::protobuf::Any& aContainer ) const
{
    ...
    const int conn = static_cast<int>( ConnectionRotationDegreesFromSpin( GetSpinStyle() ) );
    label.mutable_text()->mutable_text()->mutable_attributes()->mutable_angle()->set_value_degrees( ( conn + 180 ) % 360 );
    PackLabelBoundingBox( GetBoundingBox(), label.mutable_bbox_mm() );
    label.set_connection_rotation_degrees( conn );
