...
namespace
{
struct MODAL_SCOPE_OBSERVER
{
    const void*                           owner;
    std::function<void( bool aEntering )> fn;
};

std::vector<MODAL_SCOPE_OBSERVER>& modalScopeObservers()
{
    static std::vector<MODAL_SCOPE_OBSERVER> s_observers;
    return s_observers;
}

int g_modalScopeDepth = 0;

void notifyModalScopeBegin()
{
    if( g_modalScopeDepth++ > 0 )
        return; // already inside a modal scope; observers were already notified

    for( const MODAL_SCOPE_OBSERVER& obs : modalScopeObservers() )
        obs.fn( true );
}

void notifyModalScopeEnd()
{
    if( g_modalScopeDepth == 0 )
        return; // unbalanced; nothing to do

    if( --g_modalScopeDepth > 0 )
        return; // still inside an outer modal scope

    for( const MODAL_SCOPE_OBSERVER& obs : modalScopeObservers() )
        obs.fn( false );
}

/// RAII gate: notify observers around a DIALOG_SHIM modal / quasi-modal nested event
/// loop. Used to detach the agent-webview JS->native script messages so JavaScriptCore
/// is never entered on a KiCad tool coroutine fiber stack (which trips its stack-bounds
/// check and crashes with EXC_BREAKPOINT).
struct WEBVIEW_MODAL_GATE
{
    WEBVIEW_MODAL_GATE() { notifyModalScopeBegin(); }
    ~WEBVIEW_MODAL_GATE() { notifyModalScopeEnd(); }
};
} // namespace


void DIALOG_SHIM::AddModalScopeObserver( const void*                           aOwner,
                                         std::function<void( bool aEntering )> aObserver )
{
    modalScopeObservers().push_back( { aOwner, aObserver } );

    // If a modal scope is already active, sync the new observer into it immediately.
    if( g_modalScopeDepth > 0 )
        modalScopeObservers().back().fn( true );
}


void DIALOG_SHIM::RemoveModalScopeObserver( const void* aOwner )
{
    std::vector<MODAL_SCOPE_OBSERVER>& observers = modalScopeObservers();
...
int DIALOG_SHIM::ShowModal()
{
    ...
    WEBVIEW_MODAL_GATE webviewGate;
