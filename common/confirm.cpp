...
{
#ifdef __APPLE__
    // Use plain wxMessageDialog on macOS. wxRichMessageDialog can trigger
    // WebKit/JavaScriptCore during ShowModal when Eeschema's AI Agent WebView
    // is present, leading to EXC_BREAKPOINT in JSC::sanitizeStackForVM (e.g.
    // when opening a schematic that triggers the override-lock prompt). Plain
    // wxMessageDialog avoids that code path.
