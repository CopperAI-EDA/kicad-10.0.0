...
const wxString KIWAY::dso_search_path( FACE_T aFaceId )
{
    ...
    if( wxGetEnv( wxT( "KICAD_RUN_FROM_BUILD_DIR" ), nullptr ) )
    {
        ...
        // The build now produces CopperAI.app; fall back to the historical
        // kicad.app name so stock layouts keep working.
        wxFileName bundleProbe( fn );
        bundleProbe.AppendDir( wxT( "CopperAI.app" ) );

        if( bundleProbe.DirExists() )
            fn.AppendDir( wxT( "CopperAI.app" ) );
        else
