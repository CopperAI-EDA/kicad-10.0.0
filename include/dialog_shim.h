...
public:
    ...
    /**
     * Register/unregister an observer notified when a modal or quasi-modal dialog
     * scope begins (aEntering = true) and ends (aEntering = false). Ref-counted across
     * nested dialogs: the callbacks fire only on the outermost begin and end.
     *
     * This lets higher-level widgets pause work that is unsafe inside the nested modal
     * event loop without DIALOG_SHIM depending on them. In particular WEBVIEW_PANEL
     * uses it to detach the agent webview's JS->native script messages, which would
     * otherwise be deserialized by JavaScriptCore on a KiCad tool coroutine fiber
     * stack and crash with EXC_BREAKPOINT.
     */
    static void AddModalScopeObserver( const void*                           aOwner,
                                       std::function<void( bool aEntering )> aObserver );
    static void RemoveModalScopeObserver( const void* aOwner );
