        ...
        const HANDLER_CONTEXT<ReloadProjectFootprintLibraries>& aCtx )
{
    ...
    try
    {
        PROJECT_PCB::FootprintLibAdapter( &prj )->GlobalTablesChanged(
        ...
        const HANDLER_CONTEXT<AppendProjectFootprintLibraryRow>& aCtx )
{
    ...
    {
        response.set_ok( false );
        response.set_error_message( "No project footprint library table found" );
        return response;
    }

    LIBRARY_TABLE_ROW& row = optTbl.value()->InsertRow();
    ...
    try
    {
        auto result = optTbl.value()->Save();
        if( !result )
        {
            response.set_ok( false );
            response.set_error_message( result.error().message.ToUTF8() );
            return response;
        }
    }
    catch( const IO_ERROR& ioe )
    ...
    adapter->GlobalTablesChanged( { LIBRARY_TABLE_TYPE::FOOTPRINT } );
    broadcastFootprintLibraryReload( frame() );
